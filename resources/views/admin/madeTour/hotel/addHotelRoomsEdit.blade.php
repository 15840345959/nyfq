...
@section('content')
    ...
    <div class="page-container">
        <form class="form form-horizontal" id="form-admin-add">
            ...
            <div class="row cl hidden">
                ...
                <div class="formControls col-xs-8 col-sm-9">
                    <input id="id" name="id" type="text" class="input-text"
                           value="{{ isset($data->id) ? $data->id : '' }}" placeholder="酒店房间id">
                </div>
            </div>
            <div class="row cl">
                <label class="form-label col-xs-4 col-sm-2"><span class="c-red">*</span>酒店房间名称：</label>
                <div class="formControls col-xs-8 col-sm-9">
                    <input id="name" name="name" type="text" class="input-text"
                           value="{{ isset($data->name) ? $data->name : '' }}" placeholder="请输入酒店房间名称">
                </div>
            </div>
            <div class="row cl">
                <label class="form-label col-xs-4 col-sm-2"><span class="c-red">*</span>原价：</label>
                <div class="formControls col-xs-8 col-sm-9">
                    <input id="primecost" name="primecost" type="text" class="input-text" style="width: 100%;"
                           value="{{ isset($data->primecost) ? $data->primecost : '' }}" placeholder="请输入原价">
                </div>
            </div>
            <div class="row cl">
            ...
            <div class="row cl">
                <label class="form-label col-xs-4 col-sm-2"><span class="c-red">*</span>简单描述：</label>
                <div class="formControls col-xs-8 col-sm-9">
                    <textarea id="content" name="content" class="textarea" placeholder="请输入简单描述..." rows="" cols="" >{{ isset($data->content) ? $data->content : '' }}</textarea>
                </div>
            </div>
            <div class="row cl">
                <label class="form-label col-xs-4 col-sm-2"><span class="c-red">*</span>标签：</label>
                <div class="formControls col-xs-8 col-sm-9">
                    <textarea id="label" name="label" class="textarea" placeholder="请输入标签..." rows="" cols="" >{{ isset($data->label) ? $data->label : '' }}</textarea>
                </div>
            </div>
            <div class="row cl">
                <label class="form-label col-xs-4 col-sm-2"></label>
                <div class="formControls col-xs-8 col-sm-9">
                    <p style="color: red;">多个标签以“_”分割</p>
                </div>
            </div>
            <div class="row cl">
                <label class="form-label col-xs-4 col-sm-2"><span class="c-red">*</span>酒店名称：</label>
                <div class="formControls col-xs-8 col-sm-9">
                    <select id="hotel_goods_id" name="hotel_goods_id" class="select">
                        @foreach($hotelGoods as $hotelGoods)
                            <option value="{{ isset($hotelGoods->id) ? $hotelGoods->id : '' }}">{{ isset($hotelGoods->name) ? $hotelGoods->name : '' }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            <div class="row cl">
                <label class="form-label col-xs-4 col-sm-2"><span class="c-red">*</span>酒店房间封面图片：</label>
                <div class="formControls col-xs-8 col-sm-9">
                    <input id="image" name="image" type="text" class="input-text"
                           value="{{ isset($data->image) ? $data->image : '' }}" placeholder="请选择酒店房间封面图片">
                </div>
            </div>
            <div class="row cl">
                <label class="form-label col-xs-4 col-sm-2"></label>
        ...
        $(function () {
            ...
            $("#form-admin-add").validate({
                ...
                submitHandler: function (form) {
                    $(form).ajaxSubmit({
                        type: 'POST',
                        url: "{{ URL::asset('/admin/madeTour/hotel/editHotelRooms')}}",
