...
//            alert(str);
        ...
        function sortUp(index,id){
            ...
            if(index!=0){
                ...
                for(var i=0;i<jsonObj.length;i++){
                    editTourGoodsDetailList(jsonObj[i])
                }
                //重新展示
                refresh(jsonObj)
            }
...
//                            console.log('jsonObj.length : ' + JSON.stringify((jsonObj.length)))
        ...
        function submitDetailText() {
            ...
            if(add_text==''){
                ...
                var detail={};
                detail['tour_goods_id']='{{$data['id']}}';
                detail['content']=add_text;
                detail['type']=0;
                detail['sort']=jsonObj.length;
//                jsonObj.push(detail);
                addTourGoodsDetailList(detail,function(ret){
                    if (ret.result == true) {
        ...
        function submitDetailImage() {
            ...
            console.log('add_image:'+add_image);
            if(add_image==''){
...
//                jsonObj.push(detail);
        ...
        //提交后台编辑数据
        function editTourGoodsDetailList(jsonObj){
            var param = {
                sort:jsonObj['sort'],
            ...
            editTourGoodsDetails('{{URL::asset('')}}', param, function (ret) {
                // console.log("editTourGoodsDetails ret is ： "+JSON.stringify(ret))
                if (ret.result == true) {
                    return ret.result;
                } else {
                    layer.msg(ret.msg, {icon: 2, time: 1000})
        ...
        //提交后台添加数据
        function addTourGoodsDetailList(detail,callBack){
            var param={
                _token: "{{ csrf_token() }}",
