                ...
                <div class="row cl hidden">
                    ...
                    <div class="formControls col-xs-8 col-sm-9">
                        <input id="id" name="id" type="text" class="input-text"
                               value="{{ isset($data->id) ? $data->id : '' }}" placeholder="旅游产品id">
                    </div>
                </div>
...
//                console.log('param is : ' + JSON.stringify(param))
                delTourGoodsImage('{{URL::asset('')}}', param, function (ret) {
                    if (ret.result == true) {
...
//                            console.log('jsonObj.length : ' + JSON.stringify((jsonObj.length)))
        ...
        {{--//删除这条数据--}}
        {{--function delDetial(index,id){--}}
            {{--console.log('del1 index is : ' + JSON.stringify((jsonObj[index])))--}}
            {{--layer.confirm('确认要删除这条数据吗？',function(index){--}}
                ...
                {{--var param = {--}}
                    ...
                    {{--_token: "{{ csrf_token() }}"--}}
                {{--}--}}
                {{--console.log('del index is : ' + JSON.stringify(param))--}}
                {{--delTourGoodsImage('{{URL::asset('')}}', param, function (ret) {--}}
                    {{--if (ret.result == true) {--}}
                        {{--layer.msg(ret.msg, {icon: 1, time: 1000});--}}
...
//                jsonObj.push(detail);
                addTourGoodsImageList(detail,function(ret){
                    if (ret.result == true) {
        ...
        function editGoodsInfoDetailList(jsonObj){
            ...
            addTourGoodsImage('{{URL::asset('')}}', param, function (ret) {
                 console.log("addTourGoodsImage ret is ： "+JSON.stringify(ret))
                if (ret.result == true) {
                    return ret.result;
                } else {
                    layer.msg(ret.msg, {icon: 2, time: 1000})
        ...
        //提交后台添加数据
        function addTourGoodsImageList(detail,callBack){
            var param={
                _token: "{{ csrf_token() }}",
...
//                type:detail['type'],
            ...
            console.log("addTourGoodsImageList ret is ： "+JSON.stringify(param))
            addTourGoodsImage('{{URL::asset('')}}', param, callBack)
        }
